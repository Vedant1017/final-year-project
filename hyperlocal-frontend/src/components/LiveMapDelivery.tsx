...
export function LiveMapDelivery() {
  // Fake Bangalore-ish coords (nice for demos)
  const shops = useMemo(
    () => [
      { id: 's1', name: 'Fresh Mart', pos: [12.9722, 77.5950] as LatLng },
      { id: 's2', name: 'Green Valley Store', pos: [12.9686, 77.6096] as LatLng },
      { id: 's3', name: 'City Supermarket', pos: [12.9785, 77.5726] as LatLng },
      { id: 's4', name: 'Local Organic Grocers', pos: [12.9592, 77.5906] as LatLng },
      { id: 's5', name: 'Daily Needs Hub', pos: [12.9851, 77.6046] as LatLng }
    ],
    []
  );

  const home: { name: string; pos: LatLng } = useMemo(
    () => ({ name: 'Home (destination)', pos: [12.9719, 77.6150] }),
    []
  );
  ...
  // Fake “route” as a polyline with a couple of bends
  const route: LatLng[] = useMemo(() => {
    const a = activeShop.pos;
    const b = home.pos;
    const mid1: LatLng = [lerp(a[0], b[0], 0.35) + 0.0025, lerp(a[1], b[1], 0.35) - 0.0018];
    const mid2: LatLng = [lerp(a[0], b[0], 0.7) - 0.0018, lerp(a[1], b[1], 0.7) + 0.0022];
