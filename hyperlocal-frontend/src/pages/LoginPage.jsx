...
export function LoginPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      else if (user.role === 'OWNER' && user.sellerApproved === false) nav('/seller/pending', { replace: true });
      else if (user.role === 'OWNER') nav('/owner', { replace: true });
      else if (user.role === 'DELIVERY_MAN') nav('/delivery', { replace: true });
      else nav('/customer', { replace: true });
    } catch {
        ...
        <div className="p-7 md:p-10">
          ...
          <div className="mt-6">
            ...
            <div className="text-xs text-gray-500 mt-3">
              ...
              <span className="font-bold">admin@demo.com</span> / password123
            </div>
          </div>
          ...
          <form className="mt-8 space-y-4" onSubmit={onSubmit}>
            ...
            <div className="text-center text-sm font-semibold text-gray-600 pt-2">
              New to SnapCart?{' '}
              <Link to="/signup" className="text-brand-900 font-black hover:underline">
                Create an account
              </Link>
            </div>
